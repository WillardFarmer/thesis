...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{Mission Model.png} 
...
the Area of Interest polygon $g_{AOI}$. First the coarse imaging ground swath
is generated, $S_A$. Then the ground swath and \gls{aoi} are passed into the
Polygon Intersection Utility which return a list of access times, $A_{A}$, and
a list of intersection polygons, $G_A$. Note that the length of $A_{A}$ and
$G_A$ are the same since there cannot be an intersection polygon without an
access.  Given this property, the list of opportunities, $O$, is only the
combination of $A_{A}$ and $G_A$. Finally, the list of opportunities is
returned along with the coarse imaging ground swath. 
...
\begin{algorithm} 
	...
	\Function{TipAndCueImaging}{$E_A$, $E_B$, $g_{AOI}$, $L_G$} 
	    ...
	    \Let{$A_{A}$, \_}{\Call{PolygonIntersection}{$S_A$, $g_{AOI}$}} \Comment{Calculate only access to Polygon}
...
From this process, the handling of some edge cases have been omitted for
