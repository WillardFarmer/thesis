...
makes use of the first 3.  For GET requests, there is no payload so the only
information that is transferred is the link itself or any query parameters.
These requests are used to read information from the webserver, such as a
webpage or some specific data. DELETE requests are equivalent to GET requests
but they are used when information must be deleted from \gls{pops}.  Given how
important potential data loss is, deletions are treated separately.  POST
requests contain information in the body of the request.  The format of the
input data must be defined or else the web server will fail the request as
...
%See Algorithm~\ref{alg:crossover} for a discussion on how crossings are found.
...
to is the corresponding opportunity
...
Figure~\ref{fig:mission_model}. There are many components so this chapter will
...
Mission Model service does very few actual calculations, its only purpose is to
organize information and call functions as necessary to serve a request.
...
be interacted with the programmatically. \gls{pops} uses an open-source
\gls{orm} library. It handles the low-level functionality and allows a
developer to focus on what is unique about \gls{pops} rather than databases in
general. For example, there is no need to develop \gls{pops}'s own methods of
formulating \gls{sql} queries or parsing the output data once it is retrieved.
This has already been implemented many times over in a multitude of open-source
libraries. 
...
of the Mission Model service. It has been discussed last such that the reader
may have an understanding of the surrounding tools the opportunity filter makes
use of. It is here that the Mission Model actually searches for potential
observation opportunities and implements the filters discussed in
Section~\ref{sec:opp-filtering}. Its purpose is to combine the functionality of
the Data Handler classes and the \gls{atu} Handler class to make Opportunity
Filtering as close as possible to their high-level summaries.  In general, with
opportunity filtering, the implementation process is straightforward and
matches well with its high-level description. This is not always the case
though. Let us now revisit the Filters discussed earlier, and outline how they
are actually implemented in \gls{pops}. 
...
of intersection polygons, $G_A$. Note that the length of $A_{A}$ and $G_A$ are
the same since there cannot be an intersection polygon without an access.
...
Even though Cesium is used as the 3D Earth visualization tool, a substantial
amount of development must be done to properly configure and manage what is
being shown on the viewer. Before discussing how a Cesium viewer is managed, it
would be useful to first go over what can be displayed currently with
\gls{pops}, the Cesium Models. These model classes are not like the ones made
for \gls{orm}, rather these models have two responsibilities. They must keep
some metadata about a Cesium object and they must contain a method that
...
Now that the basic Cesium objects that are supported by \gls{pops} have been
...
To actually effect changes to the Cesium viewer, the Cesium Handler makes use
